...
class Kg4Data:
    ...
    def _find_kg4_times(self):
        ...
        ind_last_ip = len(ip_reversed) - np.argmax(
            ip_reversed > self.MIN_IP) - 1
    ...
    def read_data(self, mag, shot_no):
        ...
        for far_chan in self.constants.kg4_far.keys():
            ...
            if far_signal.data is not None:
                ...
                far_signal.delete_points(np.where((far_signal.time < mag.start_ip)|(far_signal.time > mag.end_ip)))
                ...
                if self.constants.plot_type == "dpi" and "kg4_data" in self.constants.make_plots:
                    make_plot([[far_signal.time, far_signal.data]], xtitles=["time [sec]"],
                              ytitles=["density from faraday"],
                              colours=["blue"], pformat=[1], show=True,
                              title="KG4 density from Faraday angle. Chan {}".format(far_chan))
        ...
        for ell_chan in self.constants.kg4_ell.keys():
            ...
            if ell_signal.data is not None:
                ...
                ell_signal.delete_points(np.where((ell_signal.time < mag.start_ip) | (ell_signal.time > mag.end_ip)))
                ...
                if self.constants.plot_type == "dpi" and "kg4_data" in self.constants.make_plots:
                    make_plot([[ell_signal.time, ell_signal.data]], xtitles=["time [sec]"],
        ...
        for cm_chan in self.constants.kg4_xg_elld.keys():
            ...
            if self.constants.plot_type == "dpi" and "kg4_cmdata" in self.constants.make_plots:
                make_plot([[xg_elld_signal.time, xg_elld_signal.data]],
                      xtitles=["time [sec]"],
                      ytitles=["status "],
                      colours=["blue"], pformat=[1], show=True,
        ...
        # for cm_chan in self.constants.kg4_xg-ell.keys():
                ...
                if xg_ell_signal.data is not None:
                    # Only keep points where there is ip
                    xg_ell_signal.delete_points(np.where((xg_ell_signal.time < mag.start_ip) | (xg_ell_signal.time > mag.end_ip)))
                    # DELETE POINTS WHERE BVAC <1.5T
                    xg_ell_signal.delete_points(np.where((xg_ell_signal.time < mag.start_bvac) | (
                                    xg_ell_signal.time > mag.end_bvac)))
                    #DELETE POINTS WHERE IP <1MA
                    xg_ell_signal.delete_points(np.where((xg_ell_signal.time < mag.start_ip1MA) | (
                    ...
                    if self.constants.plot_type == "dpi" and "kg4_cmdata" in self.constants.make_plots:
                        make_plot([[xg_ell_signal.time, xg_ell_signal.data*1e18]], xtitles=["time [sec]"],
