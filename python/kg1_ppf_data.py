...
class Kg1PPFData(SignalBase):
    ...
    def read_data(self, shot_no, read_uid="JETPPF"):
        ...
        for chan in self.constants.kg1r_ppf_type.keys():
            ...
            if sig_type.data is not None:
                ...
                if (
                    "kg1c" in sig_type.ihdata[ind_type:ind_chan]
                ) and chan in self.fj_met.keys():
    ...
    def set_status(self, lid, new_status, time=None, index=None):
        ...
        if time is not None and len(time) == 1:
            index = np.where(
                np.isclose(self.status[lid].time, time, atol=0.00005, rtol=1e-6)
            )[0]
        elif time is not None and len(time) == 2:
            index = [
                (np.abs(self.status[lid].time - time[0])).argmin(),
        ...
        if index is not None:
            if len(index) == 2:
                ...
                logger.debug(
                    "Chan {}: Setting status to {} between {}-{}".format(
                        lid,
                        new_status,
                ...
                logger.debug(
                    "Chan {}: Setting status to {} for time point {}".format(
                        lid, new_status, self.status[lid].time[index]
                    )
                )
